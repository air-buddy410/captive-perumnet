...
Gunakan `REYEE_AUTH_MODE=redirect` saat konfigurasi gateway telah siap. Jika gateway memakai **WiFiDog**, portal otomatis membaca `gw_address`, `gw_port`, dan `token`, lalu mengarahkan browser ke endpoint otorisasi gateway (`/wifidog/auth`). Untuk template generik, adapter tetap mengembalikan redirect ke `login_url` gateway dengan parameter username, password, dan `post_url`. Nama parameter dapat disesuaikan melalui `.env` karena firmware/template Reyee dapat berbeda. Jangan gunakan mode redirect tanpa menguji template parameter pada EG yang digunakan.
