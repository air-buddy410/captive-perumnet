...
async function loadAdminLeads() { const rows = await api('/api/admin/leads'); leads.splice(0, leads.length, ...rows.map(row => ({ name:row.full_name || 'Guest One-Click', email:row.email || 'Data tidak diisi', phone:row.phone_number || '—', address:row.address || '—', time:new Date(row.timestamp).toLocaleString('id-ID'), initials:row.full_name ? row.full_name.split(' ').slice(0,2).map(part => part[0]).join('') : 'OC', type:row.access_type === 'high_speed' ? 'high' : 'limited', verified:!!row.is_verified }))); $('#total-leads').textContent = rows.length; $('#today-leads').textContent = rows.filter(row => new Date(row.timestamp).toDateString() === new Date().toDateString()).length; renderLeads(); }
