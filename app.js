...
function renderLeads(data=leads) {
  ...
  $('#lead-rows').innerHTML = data.map(l => `<tr><td><div class="user-cell"><span class="avatar">${escapeHtml(l.initials)}</span><div><b>${escapeHtml(l.name)}</b><span>${escapeHtml(l.email)}</span>${l.type === 'high' && l.verified ? '<em class="verified-status">✓ Email terverifikasi</em>' : ''}</div></div></td><td>${escapeHtml(l.phone)}</td><td class="address-cell">${escapeHtml(l.address)}</td><td>${escapeHtml(l.ip)}</td><td><span class="access-badge ${l.type}">${access[l.type]}</span></td><td>${escapeHtml(l.ssid)}</td><td>${escapeHtml(l.time)}</td><td class="device-cell">${escapeHtml(l.mac)}</td><td><button class="delete-client" type="button" data-mac="${escapeHtml(l.mac)}" aria-label="Hapus ${escapeHtml(l.name)}">Hapus</button></td></tr>`).join('') || '<tr><td colspan="9" class="empty-state">Belum ada perangkat yang dilaporkan gateway.</td></tr>';
...
$('#quick-login-form').addEventListener('submit', async e => { e.preventDefault(); const fields = e.currentTarget.querySelectorAll('input'), button=e.currentTarget.querySelector('button'); button.disabled=true; try { const result = await api('/api/auth/login', { email:fields[0].value, password:fields[1].value, context:captiveContext }); handleAuthorization(result, () => connectToWifi(true)); } catch (error) { alert(error.message); button.disabled=false; } });
$('#open-user-login').onclick = showUserLogin; $('#back-from-user-login').onclick = showAccessChoice;
$('#user-login-form').addEventListener('submit', async e => { e.preventDefault(); const fields = e.currentTarget.querySelectorAll('input'), button=e.currentTarget.querySelector('button'); button.disabled=true; try { const result = await api('/api/auth/login', { email:fields[0].value, password:fields[1].value, context:captiveContext }); handleAuthorization(result, () => connectToWifi(true)); } catch (error) { alert(error.message); button.disabled=false; } });
$('#resend-verification').onclick = async e => { if (!pendingVerificationEmail) return alert('Masukkan kembali data pendaftaran untuk mengirim ulang email.'); e.currentTarget.disabled=true; try { const result=await api('/api/auth/resend',{ email:pendingVerificationEmail }); alert(result.message); } catch(error) { alert(error.message); } finally { e.currentTarget.disabled=false; } }; $('#back-from-verify').onclick = showAccessChoice;
